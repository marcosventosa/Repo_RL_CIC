...
def string_parser(string):
    ...
    string = string.replace(']','')
    x,y,z = string.split(',')
    x=np.interp(float(x), [340,500], [0,10])
    y=np.interp(float(y), [-100,190], [0,10])
    z=np.interp(float(z), [0,300], [0,10])
...
class RobotStudio(gym.Env):
    ...
    def step(self, action):
        ...
        positionx, positiony, positionz = self.state																			#Generamos el estado según las posiciones en x, y y z
        '''if action==0:
    ...
    '''
        ...
        info, position_s = self.sock.RS_send_info(str(action[0]),str(action[1]),str(action[2]),'1')			#Enviamos al RobotStudio los valores del movimiento en cada eje para que
                                                                                                                                                                                        #lleve a cabo la acción, y nos devuelve el valor de la posición tras dicha acción
    ...
    def reset(self):
        ...
        while info == '0':
          ...
          info , _ = self.sock.RS_send_info(str(self.x),str(self.y),str(self.z),'2')
